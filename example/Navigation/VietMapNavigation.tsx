...
const VietMapNavigationScreen = () => {
  const getGuideText = (modifier: string, type: string) => {
    if (modifier != null && type != null) {
      let data = [type.split(' ').join('_'), modifier.split(' ').join('_')];
  ...
  const getTimeArriveRemaining = () => {
    ...
    if (data < 60) setTimeArriveRemaining(`${Math.round(data)} giây`);
      ...
      setTimeArriveRemaining(
        `${day} ngày, ${hour < 10 ? '0' + hour : hour} giờ, ${
          minute < 10 ? '0' + minute : minute
        } phút`,
      );
    }
  ...
  const [routeProgressData, setRouteProgressData] =
    useState<NavigationProgressData | null>(null);
  ...
  // Markers added by tapping on the map. Each uses location.png with its 1-based index drawn on top.
  // A stable `id` (not the array index) keys each marker so removing one unmounts the right native
  // view and the others keep their identity.
  ...
  useEffect(() => {
    VietMapNavigationController.configureAlertAPI(
      'YOUR_API_KEY_ALERT_HERE',
      'YOUR_API_KEY_ID_HERE',
    );
    VietMapNavigationController.configVehicleSpeedAlert(
      'VEHICLE_ID',
      VehicleType.truck,
      5,
      1500,
  ...
  const startNavigation =
    routeData != null && !isNavigationInprogress ? (
      <View
        style={{
          borderRadius: 50,
          alignContent: 'center',
  ...
  const recenterButton =
    isOverview && routeProgressData != null ? (
      <Pressable
        onPress={() => {
  ...
  return (
    <View style={styles.container}>
        ...
        <VietMapNavigation
          ...
          onRouteProgressChange={event => {
            ...
            if (type != null && modifier != null) {
              var data = [type.replace(' ', '_'), modifier.replace(' ', '_')];
          ...
          onMapLongClick={async event => {
            VietMapNavigationController.buildRoute(
              [
                {
                  lat: 10.801674700004174,
                  long: 106.63660027352597,
                },
                {
      ...
      {startNavigation}
      {markers.length > 0 ? (
        <Pressable onPress={() => setMarkers([])} style={styles.clearButton}>
          <View style={styles.clearButtonInner}>
            <Text style={styles.clearButtonText}>
              Xoá hết ({markers.length})
            </Text>
          </View>
        </Pressable>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  markerIndex: {
    position: 'absolute',
    // Sit the number inside the circular head of the pin (upper portion of the teardrop).
    top: 0,
    color: '#f0e4e4',
    fontSize: 18,
    fontWeight: 'bold',
